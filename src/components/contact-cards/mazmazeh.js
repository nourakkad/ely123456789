...
const Mazmazeh = () => {
  ...
  // ChocoSwamp-style roles: blue = actions, purple = title, wash = soft panels
  const c = {
    blue: '#3773B8',
    blueDark: '#2A5A94',
    purple: '#524FA1',
  ...
  const prim = c.blue;
  const primHover = c.blueDark;
  const light = c.wash;
  const brand = c.purple;
  ...
  const info = {
    ...
    instagram: 'https://www.instagram.com/mazmazh.res?igsh=MXJzZGhuanQ3b2cybw==',
    instagramHandle: '@mazmazh.res',
    location:
      currentLanguage === 'AR'
  ...
  const btnBase = {
    ...
    padding: '12px',
    borderRadius: '50%',
    boxShadow: `0 2px 0 0 ${c.ink}, 0 4px 12px rgba(55, 115, 184, 0.4)`,
    transition: 'all 0.3s ease',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
      ...
      <div style={{ width: '100%', maxWidth: '360px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div
          style={{
            ...
            border: `2px solid ${prim}`,
            borderRadius: '28px',
            boxShadow: `6px 6px 0 0 ${c.ink}, 0 20px 40px rgba(55, 115, 184, 0.12), 0 8px 24px rgba(82, 79, 161, 0.1)`,
            display: 'flex',
            flexDirection: 'column',
            alignItems: 'center',
            gap: '16px',
